...
void insert(char* key, char* value, unsigned long index) {
    ...
    table[index].array[table[index].size].key = malloc(strlen(key) * sizeof(char*));
    table[index].array[table[index].size].value = malloc(strlen(value) * sizeof(char*));
...
char* get_next(char *key, int partition_number) {
    if (table[partition_number].size <= table[partition_number].get_next_index) {
        return NULL;
    } 
    if (table[partition_number].array == NULL) {
        return NULL;
    }
    if (table[partition_number].array[table[partition_number].get_next_index].key == NULL) {
        return NULL;
    }
    // int length = (strlen(table[partition_number].array[table[partition_number]
    // .get_next_index].key) > strlen(key)) ? strlen(table[partition_number].array[table[partition_number]
    // .get_next_index].key) : strlen(key);
    char *cmp_key = table[partition_number].array[table[partition_number].get_next_index].key;
...
Partitioner partition, int num_partitions) {
    ...
    //     }
    //     *args = i;
    //     if (pthread_create(&f[i], NULL, library_free, args)) {
    //         free(args);
    //         printf("Warning: pthread_creat fails!\n");
    //     }
    // }
