...
run_test () {
  ...
  if (( $testnum == 3 )); then
    # (Mod) Valgrind test
    eval valgrind --error-exitcode=26 timeout 5 $(cat $testfile) > tests-out/$testnum.valgrind.out 2> tests-out/$testnum.valgrind.err
    echo $? > tests-out/$testnum.valgrind.rc
  fi
